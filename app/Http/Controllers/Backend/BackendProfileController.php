...
class BackendProfileController extends Controller
{
    ...
    public function update_password(Request $request){
        ...
        if(Hash::check($request->old_password, auth()->user()->password)){
            ...
            toastr()->success('رمز عبور با موفقیت تغییر کرد', 'عملیات موفق');
            return redirect()->back();
        } else {
            flash()->error('رمز عبور فعلی که وارد کرده‌اید نادرست است', 'عملیات ناموفق');
            return redirect()->back();
        }  
    }
    public function update_email(Request $request){
        ...
        toastr()->success('تغییر ایمیل با موفقیت انجام شد', 'عملیات موفق');
        return redirect()->back();
    }
