...
<head>
    ...
    @if(auth()->check())
        @php
            session(['seen_notifications' => session('seen_notifications', 0)]);
...
<body style="background: #eef4f5" class="dash">
    ...
    <div class="col-12 justify-content-end d-flex">
        ...
        <div class="col-12" style="position: absolute; top: 80px; left: 10px;">
            {!! implode('', $errors->all('<div class="alert-click-hide alert alert-danger col-9 col-xl-3 rounded-0 mb-1" style="position: fixed!important; z-index: 11; opacity: .9; left: 25px; cursor: pointer;" onclick="$(this).fadeOut();">:message</div>')) !!}
        </div>
        @endif
    </div>
    @php
                        ...
                        <div class="col-12 item px-0 d-flex row">
                            ...
                            <div class="col-12 px-0">
                                <ul class="sub-item font-1" style="list-style: none;">
                                    @can('categories-read')
                                    <li><a href="{{ route('admin.categories.index') }}" style="font-size: 16px;"><span class="fal fa-tag px-2" style="width: 28px; font-size: 15px;"></span> دسته‌بندی‌ها</a></li>
                                    @endcan
                                    @can('articles-read')
                                    <li><a href="{{ route('admin.articles.index') }}" style="font-size: 16px;"><span class="fal fa-book px-2" style="width: 28px; font-size: 15px;"></span> مقالات</a></li>
                                    @endcan
                                    @can('comments-read')
                                    <li><a href="{{ route('admin.article-comments.index') }}" style="font-size: 16px;"><span class="fal fa-comments px-2" style="width: 28px; font-size: 15px;"></span> نظرات
                                        @php
                                    ...
                                    <li><a href="{{ route('admin.announcements.index') }}" style="font-size: 16px;"><span class="fal fa-bullhorn px-2" style="width: 28px; font-size: 15px;"></span> اطلاعیه‌ها</a></li>
                                    @endcan
                                    @can('pages-read')
                                    <li><a href="{{ route('admin.pages.index') }}" style="font-size: 16px;"><span class="fal fa-file-invoice px-2" style="width: 28px; font-size: 15px;"></span> صفحات</a></li>
                                    @endcan
                                    @can('menus-read')
                                    <li><a href="{{ route('admin.menus.index') }}" style="font-size: 16px;"><span class="fal fa-list px-2" style="width: 28px; font-size: 15px;"></span> منوها</a></li>
                                    @endcan
                                    @can('faqs-read')
                                    <li><a href="{{ route('admin.faqs.index') }}" style="font-size: 16px;"><span class="fal fa-question px-2" style="width: 28px; font-size: 15px;"></span> سوالات متداول</a></li>
                                    @endcan
                                    @can('redirections-read')
                                    <li><a href="{{ route('admin.redirections.index') }}" style="font-size: 16px;"><span class="fal fa-directions px-2" style="width: 28px; font-size: 15px;"></span> تغییر مسیرها</a></li>
                                    @endcan
                                    @can('tags-read')
                                    <li><a href="{{ route('admin.tags.index') }}" style="font-size: 16px;"><span class="fal fa-tags px-2" style="width: 28px; font-size: 15px;"></span> برچسب‌ها</a></li>
                                    @endcan
                                </ul>
                            </div>
                        </div>
                    ...
                    <div class="col-12 px-0" style="cursor: pointer;">
                        <div class="col-12 item px-0 d-flex row">
                            ...
                            <div class="col-12 px-0">
                                <ul class="sub-item font-1" style="list-style: none;">
                                    @can('plugins-read')
                                    <li><a href="{{ route('admin.plugins.index') }}" style="font-size: 16px;"><span class="fal fa-box-open px-2" style="width: 28px; font-size: 15px;"></span> همه افزونه‌ها                                        @if(count($plugins))
                                    ...
                                    @foreach($plugins as $plugin)
                                        @if($plugin->get('type') == "plugin")
                                            @can($plugin->get('route') . '-read')
                                            <li><a href="{{ route('admin.teams.index') }}" style="font-size: 16px') }}" style="font-size: 16px;"><span class="{{ $plugin->get('icon') }} px-2" style="width: 28px; font-size: 15px;"></span> {{ $plugin->get('title') }}</a></li>
                                            @endcan
                                        @endif
                                    @endforeach
                                </ul>
                ...
                <div class="col-12 px-0 d-flex justify-content-end" style="height: 60px;">
                    <div class="btn-group" id="notificationDropdown">
                        <div class="col-12 px-0 d-flex justify-content-center align-items-center btn" style="width: 55px; height: 55px;" data-bs-toggle="dropdown" aria-expanded="false" id="dropdown-notifications">
                            ...
                            <span style="position: absolute; min-width: 25px; min-height: 25px; @if($unreadNotifications != 0) display: inline-block; @else display: none; @endif right: 0px; top: 0px; border-radius: 20px; background: #c00; color: #fff; font-size: 14px;" class="text-center" id="dropdown-notifications-icon">{{ $unreadNotifications }}</span>
                        </div>
                            ...
                            <div class="col-12 d-flex border-top">
                                <a href="{{ route('admin.notifications.index') }}" class="d-block py-2 px-3">
                                    <div class="col-12 align-items-center">
                                        <span class="fal fa-bells"></span> نمایش همه اعلان‌ها
                                    </div>
                                </a>
                            </div>
                        </div>
                    ...
                    <div class="col-12 px-0 d-flex justify-content-center align-items-center dropdown" style="width: 55px; height: 55px;">
                        ...
                        <ul class="dropdown-menu shadow border-0" aria-labelledby="dropdownMenuButton1" style="top: -3px;">
                            <li><a class="dropdown-item font-1" href="/" target="_blank"><span class="fal fa-desktop font-1"></span> نمایش سایت</a></li>
                            <li><a class="dropdown-item font-1" href="{{ route('admin.profile.index') }}"><span class="fal fa-user font-1"></span> پروفایل من</a></li>
                            <li><a class="dropdown-item font-1" href="{{ route('admin.profile.edit') }}"><span class="fal fa-edit font-1"></span> ویرایش پروفایل من</a></li>
                            @can('hub-files-read')
                            <li><a class="dropdown-item font-1" href="{{ route('admin.files.index') }}"><span class="fal fa-file font-1"></span> فایل‌ها</a></li>
                            @endcan
                            @can('traffics-read')
                            <li><a class="dropdown-item font-1" href="{{ route('admin.traffics.index') }}"><span class="fal fa-traffic-light font-1"></span> ترافیک</a></li>
                            @endcan
                            @can('error-reports-read')
                            <li><a class="dropdown-item font-1" href="{{ route('admin.traffics.error-reports') }}"><span class="fal fa-bug font-1"></span> گزارش‌های خطا</a></li>
                            @endcan
                            <li><hr style="height: 1px; margin: 10px 0px 5px;"></li>
                            <li><a class="dropdown-item font-1" href="#" onclick="document.getElementById('logout-form').submit();"><span class="fal fa-sign-out-alt font-1"></span> خروج</a></li>
                        </ul>
                    </div>
