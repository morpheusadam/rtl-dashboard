		...
		<div class="col-12 px-0">
			<div class="col-12 p-0 row">
				<div class="col-12 col-lg-4 py-3 px-3">
					<span class="fas fa-contacts"></span> تماس‌ها
				</div>
			...
			<table class="table table-bordered  table-hover">
				<thead>
					<tr>
						<th>#</th>
						@can('contacts-update')
						<th scope="col" style="width:40px;">انجام شد</th>
						@endcan
						<th>کاربر</th>
						<th>محتوای تیکت</th>
						<th>کنترل</th>
					...
					class="@if($contact->status=="DONE") ticket-resolved @endif" 
					 >
						...
						<td>
							...
							@if($contact->user_id!=null)
	                        	...
	                        	<a href="{{route('admin.users.show',$contact->user)}}" class="d-inline-block text-center">
	                                <img src="{{$contact->user->getUserAvatar()}}" style="width: 45px;height: 45px;display: inline-block;border-radius: 50%!important;padding: 3px;" class="mx-auto" alt="تصویر کاربر">
	                                <span style="display: inline-block;position: relative;top: 6px; " class="px-2 pt-0  text-start kufi">{{$contact->user->name}}</span>
	                            </a> 
	                            @else
	                            
	                                <img src="https://manager.almadarisp.com/user/img/user.png" style="width: 45px;height: 45px;display: inline-block;border-radius: 50%!important;padding: 3px;" class="mx-auto" alt="تصویر کاربر">
						...
						<td>{{mb_strimwidth($contact->message,0,80,'...')}}
							...
							@if($last_reply!=null)
								{{
									$last_reply->is_support_reply==1?"پشتیبانی فنی":$contact->name
								}}
								<br>
								{{
						...
						<td style="width: 180px;">
							...
							<span class="btn  btn-success btn-sm font-1 mx-1">
								<span class="fal fa-paper-plane"></span> پیام
							</span>
							</a>
							@endcan
							...
							<form method="POST" action="{{route('admin.contacts.destroy',$contact)}}" class="d-inline-block">@csrf @method("DELETE")
								<button class="btn  btn-outline-danger btn-sm font-1 mx-1" onclick="var result = confirm('آیا از حذف مطمئن هستید؟');if(result){}else{event.preventDefault()}">
