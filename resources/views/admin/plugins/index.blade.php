					...
					<div class="col-12 p-0">
						...
						<div class="col-12 px-0 py-3 text-center row" style="text-align: justify;">
							...
							<form method="POST" action="{{route('admin.plugins.deactivate',['plugin'=>$plugin->get('name')])}}" class="d-inline-block mb-2 col-lg-12 col-12 p-1 text-center">@csrf
								<button class="btn btn-outline-warning rounded-0 px-4" onclick="var result = confirm('آیا از غیرفعال کردن افزونه مطمئن هستید؟');if(result){}else{event.preventDefault()}">
									غیرفعال کردن افزونه
								</button>
							</form>
							@else
							<form method="POST" action="{{route('admin.plugins.activate',['plugin'=>$plugin->get('name')])}}" class="d-inline-block mb-2 col-lg-12 col-12 p-1 text-center">@csrf
								<button class="btn btn-outline-success rounded-0 px-4" onclick="var result = confirm('آیا از فعال کردن افزونه مطمئن هستید؟');if(result){}else{event.preventDefault()}">
									فعال کردن افزونه
								</button>
							</form>
							@endif
							...
							{{-- <form method="POST" action="{{route('admin.plugins.delete',['plugin'=>$plugin->get('name')])}}" class="d-inline-block mb-2 col-12 col-lg-5 p-1">@csrf
								<button class="btn btn-outline-danger rounded-0 px-2 col-12" onclick="var result = confirm('آیا از حذف افزونه مطمئن هستید؟');if(result){}else{event.preventDefault()}">
									حذف افزونه
								</button>
							</form> --}}
