		...
		<div class="col-12 px-0">
			<div class="col-12 p-0 row">
				<div class="col-12 col-lg-4 py-3 px-3">
					<span class="fas fa-files"></span> مدیر فایل‌ها
				</div>
					...
					<tr>
						...
						<td class="text-truncate d-flex">
							...
							<a href="{{$file->getUrl()}}" style="display: inline-block;" target="_blank">
							 <span class="fas fa-link mx-1"></span>	لینک
							</a>
							<br>
							@if($file->views!=0)
							...
							@if($file->last_access!=null)
							 <span class="fas fa-clock mx-1"></span>آخرین دسترسی {{\Carbon::parse($file->last_access)->diffForHumans()}}
							 <br>
							@endif
						...
						<td style="width: 180px;">
							...
							<span class="btn  btn-outline-success btn-sm font-1 mx-1 py-1 px-2">
								<span class="fas fa-eye "></span> نمایش
							</span>
							</a>
							@endcan
							@can('hub-files-delete')
							<form method="POST" action="{{route('admin.files.destroy',$file)}}" class="d-inline-block">@csrf @method("DELETE")
								<button class="btn  btn-outline-danger btn-sm font-1 mx-1 py-1 px-2" onclick="var result = confirm('آیا از حذف مطمئن هستید؟');if(result){}else{event.preventDefault()}">
